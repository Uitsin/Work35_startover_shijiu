  ...
  Dump(lmp, narg, arg)
{
  ...
  format_default = new char[6*size_one+1];
  format_default[0] = '\0';

  for (int i = 0; i < size_one; i++) {
    //    if (vtype[i] == INT) format_default = strcat(format_default,"%5.0d ");
    //    else format_default = strcat(format_default,"%8.3f ");
...
void DumpChLocal::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"index") == 0) {
      pack_choice[i] = &DumpChLocal::pack_index;
      vtype[i] = INT;
    ...
    // if no trailing [], then arg is set to 0, else arg is int between []
      ...
      computefixflag = 1;
      pack_choice[i] = &DumpChLocal::pack_compute;
      vtype[i] = DOUBLE;
    ...
    // if no trailing [], then arg is set to 0, else arg is between []
      ...
      computefixflag = 1;
      pack_choice[i] = &DumpChLocal::pack_fix;
      vtype[i] = DOUBLE;

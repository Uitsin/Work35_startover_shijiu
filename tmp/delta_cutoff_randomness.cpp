...
#include "string.h"
#include "math.h"
#include "delta_cutoff_randomness.h"
#include "group.h"
#include "modify.h"
#include "error.h"
#include "atom.h"
...
DeltaCutoffRandomness::DeltaCutoffRandomness(LAMMPS *lmp) : Pointers(lmp) {}

void DeltaCutoffRandomness::command(int narg, char **arg)
{
  ...
  for (n = 0; n < nlocal; n++){
    ...
    //    fprintf(screen, "fmod (2 , 2.0) = %f and check fmod (2 , 2.0) == 0  %d \n",fmod(2,2.0), fmod(2,2.0)==0);
    //    fprintf(screen, "fmod (-2 , 2.0) = %f and check fmod (-2 , 2.0) == 0  %d \n",fmod(-2,2.0), fmod(-2,2.0)==0);
    check_x = (fmod(fabs(channel_x-0.5), 2.0) ==0);
    check_y = (fmod(fabs(channel_y-0.5), 2.0) ==0);
    check_z = (fmod(fabs(channel_z-0.5), 2.0) ==0);
...
int DeltaCutoffRandomness::find_channel_atom(int rock_atom1, int rock_atom2){
